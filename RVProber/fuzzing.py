...
hill_climbing_iter = 200
#hill_climbing_hold_time = 15
hill_climbing_hold_time = 15
logs_path = ""
target_flight_mode = 5
environment_mutate = "off"
...
# Global Variables for the RVProber
lat_error_max = 0.0
lon_error_max = 0.0
alt_error_max = 0.0
roll_error_max = 0.0
...
def handle_target(msg):
    ...
    global alt_error_max
    global roll_error_max
    global pitch_error_max
    global heading_error_max
    ...
    if alt_error > alt_error_max:
        alt_error_max = alt_error

    if roll_error > roll_error_max:
        roll_error_max = roll_error

    if pitch_error > pitch_error_max:
        pitch_error_max = pitch_error

    if heading_error > heading_error_max:
        heading_error_max = heading_error
...
def handle_position(msg):
    ...
    global home_lat
    global home_lon
    global lat_error_max
    global lon_error_max
    ...
    # Code for RVProber
    if start_measure == "yes":
        #print("[DEBUG]home_lat:%f, msg.lat:%f" %(home_lat, msg.lat))

        lat_error = abs(home_lat - msg.lat)
        lon_error = abs(home_lon - msg.lon)

        if lat_error > lat_error_max:
            lat_error_max = lat_error

        if lon_error > lon_error_max:
            lon_error_max = lon_error
...
def handle_status(msg):
    ...
    global PreArm_error
    global Baro_status
    global EKF_lane_switch
    global GPS_glitch
    global failsafe
    global crash_on_ground
    global vibration_compensation
    global disarming_motors
    ...
    # Detecting "EKF2/3 lane switch"
    elif "lane switch" in msg.text:
        EKF_lane_switch = 1

    # Detecting "GPS Glitch"
    elif "GPS Glitch" in msg.text:
        GPS_glitch = 1

    # Detecting "failsafe"
    elif ("Failsafe" in msg.text) or ("failsafe" in msg.text):
        failsafe = 1

    # Detecting "crash on the ground"
    elif "Hit ground at" in msg.text:
        crash_on_ground = 1

    # Detecting "disarming motors"
...
# ------------------------------------------------------------------------------------
def change_parameter_value(name, value):
    master.mav.param_set_send(master.target_system, master.target_component,
                              name,
                              value,
                              mavutil.mavlink.MAV_PARAM_TYPE_REAL32)
...
# ------------------------------------------------------------------------------------
def gps_spoofing():
    global hill_climbing_iter
    global hill_climbing_hold_time

    if intensity_of_attack >= 1:
        change_parameter_value('SIM_GPS_NOISE', 3*intensity_of_attack)
        change_parameter_value('SIM_GPS_VERR_X', 3*intensity_of_attack)
        change_parameter_value('SIM_GPS_VERR_Y', 3*intensity_of_attack)
        change_parameter_value('SIM_GPS_VERR_Z', 3*intensity_of_attack)
        change_parameter_value('SIM_GPS_GLITCH_X', 0.00002*intensity_of_attack)
        change_parameter_value('SIM_GPS_GLITCH_Y', 0.00002*intensity_of_attack)
        change_parameter_value('SIM_GPS_GLITCH_Z', 0.00002*intensity_of_attack)
        change_parameter_value('SIM_GPS_NUMSATS', 5)

    elif intensity_of_attack < 0:
        change_parameter_value('SIM_GPS_NUMSATS', 5)
        for iteration in range(2):
            if iteration == 0:
                range_list = range(hill_climbing_iter+1)
            elif iteration == 1:
                range_list = range(hill_climbing_iter + 1, -1, -1)

            for i in range_list:
                change_parameter_value('SIM_GPS_NOISE', (abs(3*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GPS_VERR_X', (abs(3*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GPS_VERR_Y', (abs(3*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GPS_VERR_Z', (abs(3*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GPS_GLITCH_X', (abs(0.00002*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GPS_GLITCH_Y', (abs(0.00002*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GPS_GLITCH_Z', (abs(0.00002*intensity_of_attack)/hill_climbing_iter)*i)
                time.sleep(0.1)

            time.sleep(hill_climbing_hold_time)

# ------------------------------------------------------------------------------------
def gps_jamming():
    change_parameter_value('SIM_GPS_DISABLE', 1)
    change_parameter_value('SIM_GPS2_DISABLE', 1)
...
def spoofing_gyro():
    ...
    # Turn on the enhanced filter
    change_parameter_value('INS_HNTCH_ENABLE', 1)   # Turning on Harmonic Notch Filter
    change_parameter_value('INS_HNTC2_ENABLE', 1)
    change_parameter_value('INS_HNTCH_MODE', 4)     # Setting dynamic frequency tracking mode
    change_parameter_value('INS_HNTC2_MODE', 4)


    if intensity_of_attack >= 1:
        change_parameter_value('SIM_GYR_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_GYR1_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_GYR2_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_GYR3_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC1_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC2_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC3_RND', 400*intensity_of_attack)
        ...
        for iteration in range(2):
            ...
            for i in range_list:
                change_parameter_value('SIM_GYR_RND', (abs(400*intensity_of_attack) / hill_climbing_iter)*i)
                change_parameter_value('SIM_GYR1_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GYR2_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_GYR3_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC_RND', (abs(400*intensity_of_attack) / hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC1_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC2_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC3_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                time.sleep(0.1)
...
def spoofing_mag():
    ...
    if intensity_of_attack >= 1:
        change_parameter_value('SIM_MAG_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC_RND', 400 * intensity_of_attack)
        change_parameter_value('SIM_ACC1_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC2_RND', 400*intensity_of_attack)
        change_parameter_value('SIM_ACC3_RND', 400*intensity_of_attack)
        ...
        for iteration in range(2):
            ...
            for i in range_list:
                change_parameter_value('SIM_MAG_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC_RND', (abs(400 * intensity_of_attack) / hill_climbing_iter) * i)
                change_parameter_value('SIM_ACC1_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC2_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_ACC3_RND', (abs(400*intensity_of_attack)/hill_climbing_iter)*i)
...
# ------------------------------------------------------------------------------------
def disconnecting_network():
    global target_vehicle_type

    if target_vehicle_type == "ArduCopter":
        change_parameter_value('FS_GCS_ENABLE', 5)   # Landing when the GCS failsafe is triggered
    elif target_vehicle_type == "Rover":
        change_parameter_value('FS_GCS_ENABLE', 1)   # mode will be 'hold' during the GCS failsafe
    elif target_vehicle_type == "ArduSub":
        change_parameter_value('FS_GCS_ENABLE', 2)   # Disarming motors during the GCS failsafe

    change_parameter_value('FS_GCS_TIMEOUT', -1) # Timeout before triggering the GCS failsafe
    change_parameter_value('FS_OPTIONS', 0)  # Trigger the GCS failsafe regardless of the pilot's RV controller

# ------------------------------------------------------------------------------------
def spoofing_optical_flow():
    global hill_climbing_iter
    global hill_climbing_hold_time

    if intensity_of_attack >= 1:
        change_parameter_value('SIM_FLOW_RND', 3*intensity_of_attack)
        change_parameter_value('SIM_FLOW_DELAY', 3*intensity_of_attack)
        ...
        for iteration in range(2):
            ...
            for i in range_list:
                change_parameter_value('SIM_FLOW_RND', (abs(3*intensity_of_attack)/hill_climbing_iter)*i)
                change_parameter_value('SIM_FLOW_DELAY', (abs(3*intensity_of_attack)/hill_climbing_iter)*i)
                time.sleep(0.1)
...
def environment_change():
    ...
    change_parameter_value('SIM_WIND_SPD', 15)
    time.sleep(0.2)
    change_parameter_value('SIM_WAVE_ENABLE', 1)
    time.sleep(0.2)
    change_parameter_value('SIM_WAVE_SPEED', 10)
    time.sleep(0.2)

    while True:
        val = random.randint(1, 360)
        change_parameter_value('SIM_WIND_DIR', val)
        time.sleep(0.2)
        change_parameter_value('SIM_WAVE_DIR', val)
        time.sleep(0.2)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "ha:pt:n:v:i:d:f:s:e", ["attack_type=","test_time=","under_normal=","vehicle_type=","intensity_of_attack=","directory=","mode=","--sw_version"])
    ...
    for opt, arg in opts:
        ...
        if opt in ("-f", "--mode"):
            target_flight_mode = int(arg)
            print("[fuzzing.py] target flight mode:%s" % target_flight_mode)
        if opt in ("-s", "--sw_version"):
            software_path = str(arg)
        if opt == '-e':
            environment_mutate = "on"
    outfile_name = '/' + 'a=' + str(attack_type) + ',' + 'i=' + str(intensity_of_attack) + ',' + 'v=' + str(target_vehicle_type) + ',' \
                   + 'f=' + str(target_flight_mode) + ',' + 't=' + str(test_time) + ',' + str(software_path)

    hill_climbing_hold_time = test_time/4
    ...
    # Turn on all logging features
    change_parameter_value('LOG_BITMASK', 983039)
    time.sleep(0.2)
    print("[DEBUG] Turn on all logging features")

    # Let's change environment conditions if users turn on this feature.
    if environment_mutate == "on":
        thread_env = threading.Thread(target=environment_change, args=())
        thread_env.daemon = True
        thread_env.start()
    ...
    if target_vehicle_type == "ArduCopter":
        ...
        #time.sleep(25) # It makes the RV reach around 50 meters of altitude.
        time.sleep(15) # It makes the RV reach around 30 meters of altitude.
    ...
    # FLIP flight mode requires the RV to be in ALT_HOLD flight mode.
    if target_vehicle_type == "ArduCopter" and target_flight_mode == 14:
        master.mav.set_mode_send(
            master.target_system,
            mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,
            2)
    ...
    # Update lat and lon of the home positions
    message = master.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
    home_lat = message.lat
    home_lon = message.lon
    start_measure = "yes"

    # Set preconditions before measuring the RV's physical states
    """
    master.mav.param_set_send(master.target_system, master.target_component,
                              'SIM_GPS_TYPE',
                              2,
                              mavutil.mavlink.MAV_PARAM_TYPE_REAL32)
    """
    # To-do: Add other attack types
    # ----- (start) Let's start to profile the selected attack. -----
    #if attack_profile == "on" and under_normal == "no":
    if under_normal == "no":
        if attack_type == 'GPS_spoofing':
            print("[probing.py] User chooses GPS spoofing attack.")
            #gps_spoofing()
            thread_attack = threading.Thread(target=gps_spoofing, args=())
            thread_attack.daemon = True
            thread_attack.start()
        elif attack_type == 'GPS_jamming':
            #gps_jamming()
            thread_attack = threading.Thread(target=gps_jamming, args=())
            thread_attack.daemon = True
            thread_attack.start()
            print("[probing.py] User chooses GPS jamming attack.")
        elif attack_type == 'spoofing_gyro':
            #spoofing_gyro()
            thread_attack = threading.Thread(target=spoofing_gyro, args=())
            thread_attack.daemon = True
            thread_attack.start()
            print("[probing.py] User chooses gyroscope spoofing attack.")
        elif attack_type == 'spoofing_mag':
            #spoofing_mag()
            thread_attack = threading.Thread(target=spoofing_mag, args=())
            thread_attack.daemon = True
            thread_attack.start()
            print("[probing.py] User chooses magnetometer spoofing attack.")
        elif attack_type == 'disconnecting_network':
            #disconnecting_network()
            thread_attack = threading.Thread(target=disconnecting_network, args=())
            thread_attack.daemon = True
            thread_attack.start()
            print("[probing.py] User chooses disconnecting_network attack.")
        elif attack_type == 'spoofing_opticalflow':
            #spoofing_optical_flow()
            thread_attack = threading.Thread(target=spoofing_optical_flow, args=())
            thread_attack.daemon = True
            thread_attack.start()
            print("[probing.py] User chooses opticalflow attack.")
    # ----- (end) Let's start to profile the selected attack. -----
    ...
    while True:
        ...
        if under_normal == "yes":
            print("----------------[Under the normal]-------------------")
        elif under_normal == "no":
            print("----------------[Under the attack]-------------------")

        # *Check whether the attack is successful or not*
        attack_result = "attack_fail"
        if (GPS_glitch == 1) or (failsafe == 1) or (crash_on_ground == 1) or (disarming_motors == 1):
            attack_result = "attack_success"

        # ------------- For logging the attack result -------------
        open("attack_result.txt", "w").close()
        f = open("attack_result.txt", "w")

        if attack_result == "attack_success":
            f.write("attack_success")
        elif attack_result == "attack_fail":
            f.write("attack_fail")
        ...
        print("[probing.py] Current testing time: %d, The maximum testing time: %d" %(time_counter, test_time))
        print("[probing.py] lat_error_max:%f, lon_error_max:%f, alt_error_max:%f, roll_error_max:%f, pitch_error_max:%f, heading_error_max:%f, "
              "vibration_compensation:%d, EKF_lane_switch:%d, GPS_glitch:%d, failsafe:%d, crash_on_ground:%d, disarming_motors:%d, attack_result:%s"
              %(lat_error_max, lon_error_max, alt_error_max, roll_error_max, pitch_error_max, heading_error_max, vibration_compensation, EKF_lane_switch, GPS_glitch, failsafe, crash_on_ground, disarming_motors, attack_result))
        print("-----------------------------------")
        if time_counter >= test_time:

            # Log the maximum errors of RV physical states
            if under_normal == "yes":

                open(logs_path + outfile_name + "_normal.txt", "w").close()
                attack_profile_file = open(logs_path + outfile_name + "_normal.txt", "w")
            elif under_normal == "no":
                open(logs_path + outfile_name + "_attack.txt", "w").close()
                attack_profile_file = open(logs_path + outfile_name + "_attack.txt", "w")

            if under_normal == "yes":
                attack_profile_file.write('# [Under the normal] lat_error_max, lon_error_max, alt_error_max, roll_error_max, pitch_error_max, yaw_error_max'
                                        ', vibration_compensation, EKF_lane_switch, GPS_glitch, failsafe, crash_on_ground, disarming_motors, attack_result\n')
            elif under_normal == "no":
                attack_profile_file.write('# [Under the attack] lat_error_max, lon_error_max, alt_error_max, roll_error_max, pitch_error_max, yaw_error_max'
                                        ', vibration_compensation, EKF_lane_switch, GPS_glitch, failsafe, crash_on_ground, disarming_motors, attack_result\n')

            attack_profile_file.write(str(lat_error_max) + ',' + str(lon_error_max) + ',' + str(alt_error_max) + ',' + str(roll_error_max) + ',' + str(pitch_error_max)
                                      + ',' + str(heading_error_max) + ',' + str(vibration_compensation) + ',' + str(EKF_lane_switch) + ',' + str(GPS_glitch) + ','
                                      + str(failsafe) + ',' + str(crash_on_ground) + ',' + str(disarming_motors) + ',' + str(attack_result) + '\n')
            attack_profile_file.close()
            break

    open("simulator_states.txt", "w").close()
    simulator_states = open("simulator_states.txt", "w")
    if under_normal == "yes":
        simulator_states.write('Complete under the normal')
    elif under_normal == "no":
        simulator_states.write('Complete under the attack')
    simulator_states.close()
